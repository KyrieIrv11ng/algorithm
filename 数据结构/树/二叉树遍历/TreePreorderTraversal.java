...
public class TreePreorderTraversal {
    ...
    public static void preorder(leetcode.TreeNode root, ArrayList<Integer> al){
        if(root==null){
            return;
        }
        al.add(root.val);
    ...
    //2.迭代方法
    public List<Integer> preorderTraversal1(leetcode.TreeNode root) {
        ArrayList<Integer> result = new ArrayList<>();
        preOrderIteration(root,result);
        return result;
    }
    public static void preOrderIteration(leetcode.TreeNode root, ArrayList<Integer> result){
        if(root == null){
            return;
        }
