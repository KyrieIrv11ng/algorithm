...
public class TreeInorderTraversal {
    ...
    public static void inorder(leetcode.TreeNode root, ArrayList<Integer> al){
        if(root==null){
            return;
        }
    ...
    //2.迭代方法
    public List<Integer> inorderTraversal1(leetcode.TreeNode root) {
        ArrayList<Integer> al = new ArrayList<>();
        inorderIteration(root,al);
        return al;
    }
    public static void inorderIteration(leetcode.TreeNode root, ArrayList<Integer> res){
        if(root == null){
            return;
        }
        Stack<leetcode.TreeNode> s = new Stack<>();
        leetcode.TreeNode cur = root; //标记当前节点
